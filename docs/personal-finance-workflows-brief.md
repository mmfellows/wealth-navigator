...
- [ ] **P2 — AI categorization + review queue:** `aiCategorizationService.js`, post-sync hook + on-demand endpoint, review-queue API, review-card UI (mobile-friendly).
- [ ] **P3 — Monthly close wizard** (`/close`, five steps, server-side month close, Reports pacing switched off localStorage).
- [ ] **P4 — Mobile + weekly pacing:** responsive `Layout.tsx`, `/pacing` page, PWA manifest, burn-vs-income strip (F4) on `/reports` + `/pacing`, trailing aggregates in stats endpoint.
- [ ] **P5 — Finance chat** (route + tool-runner service + streaming + chat UI).
- [ ] **P6 — New-cost radar:** sync-time flagging, radar UI on `/pacing` + `/close`, acknowledged state; then recurring detection (own or Plaid recurring endpoint).
