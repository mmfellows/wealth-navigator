...
|---|---|---|---|
| `ENCRYPTION_KEY` | App-layer AES-256 of Plaid access tokens (and E*TRADE consumer keys) | At least annually + on suspected compromise | Vercel never reveals a stored Sensitive value, so rotation runs inside the app. (1) Generate a new 32-byte hex key. (2) Set it as `ENCRYPTION_KEY_NEXT` in Vercel and deploy: new writes use it, reads fall back to it. (3) `POST /api/internal/rotate-encryption-key` (cron secret) re-encrypts every stored secret; `GET /api/internal/encryption-key-status` shows which key each row is readable with, without decrypting values. (4) Set `ENCRYPTION_KEY` to the new value, remove `ENCRYPTION_KEY_NEXT`, deploy. (5) Verify with `/api/internal/sync-all`. Each row carries `encryption_key_id` (first 8 hex of SHA-256 of the key) so the state is auditable. Last rotated: 2026-09-14. |
